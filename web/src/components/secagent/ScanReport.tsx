  ...
  return (
    <div className="relative mx-auto h-36 w-36">
      <svg className="-rotate-90 transform" viewBox="0 0 120 120">
        <circle cx="60" cy="60" r={r} fill="none" stroke="#1a1f1a" strokeWidth="8" />
        <circle
          cx="60"
          cy="60"
          r={r}
  ...
  return (
    <div className="animate-fade-up relative z-10 mx-auto max-w-5xl px-6 pb-32">
      <div className="mb-10 grid gap-8 rounded-xl border border-border bg-surface/80 p-8 backdrop-blur-sm md:grid-cols-[200px_1fr]">
        <RiskRing score={report.risk_score} />
        <div>
          <div className="font-mono text-xs text-green-bright">// {report.target}</div>
          ...
          <div className="mt-6 flex h-3 w-full overflow-hidden rounded-full bg-border">
            {counts.CRITICAL > 0 && (
              <div className="bg-risk-critical" style={{ width: w(counts.CRITICAL) }} title="Critical" />
            )}
            {counts.HIGH > 0 && (
              <div className="bg-risk-high" style={{ width: w(counts.HIGH) }} title="High" />
            )}
            {counts.MEDIUM > 0 && (
              <div className="bg-risk-medium" style={{ width: w(counts.MEDIUM) }} title="Medium" />
            )}
            {counts.LOW > 0 && (
              <div className="bg-risk-low" style={{ width: w(counts.LOW) }} title="Low" />
            )}
            {counts.INFO > 0 && (
              <div className="bg-risk-info" style={{ width: w(counts.INFO) }} title="Info" />
            )}
          </div>
      ...
      <h2 className="mb-4 font-display text-2xl font-semibold text-text">Findings</h2>
